...
function isCreditCardNumber(ccn) {
  const startPos = !(`${ccn}`.length % 2) ? 0 : 1;
  let checkSum = Number(startPos ? `${ccn}`[0] : 0);
  for (let i = startPos; i < `${ccn}`.length; i += 2) {
    checkSum += Number(`${ccn}`[i] * 2 > 9 ? `${ccn}`[i] * 2 - 9 : `${ccn}`[i] * 2);
    checkSum += Number(`${ccn}`[i + 1]);
  }
  return !(checkSum % 10);
}
